...
## Population Parameters vs Sample Statistics {.smaller}
...
Let's say we have 5 numbers with a mean of 10:

::: {.incremental}
- If we know four numbers: 8, 9, 11, 12
- The fifth number MUST be 10 to maintain the mean of 10
- We can calculate it: $5 \times 10 - (8 + 9 + 11 + 12) = 10$
- This loss of freedom is why we use $n-1$
- Using $n-1$ gives an unbiased estimate of population variance
:::

## R Simulation: n vs n-1 {.smaller}
...
pop_var <- var(population) * (length(population)-1)/length(population)

# Function to compare variance estimates
compare_var <- function(n_samples = 1000, sample_size = 30) {
  samples <- replicate(n_samples, {
    sample <- sample(population, size = sample_size)
    c(
      n1 = var(sample),  # using n-1
      n = var(sample) * (length(sample)-1)/length(sample)  # using n
    )
  })
...
# Compare estimates
estimates <- compare_var()
cat("Population variance:", pop_var, "\n",
    "Mean estimate (n-1):", estimates["n1"], "\n",
    "Mean estimate (n):", estimates["n"], "\n")
```
